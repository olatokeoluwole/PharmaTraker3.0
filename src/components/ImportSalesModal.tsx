...
export default function ImportSalesModal({ isOpen, onClose, onSuccess, drugs }: ImportSalesModalProps) {
  ...
  const handleDownloadTemplate = () => {
    const csvContent = "data:text/csv;charset=utf-8,Date,DrugName,BranchName,Quantity,UnitPrice,TotalAmount,PatientName,PaymentMethod\n2023-10-01T14:30:00,Paracetamol,Central Branch,2,800,1600,John Doe,cash\n2023-10-02T09:15:00,Amoxicillin,Central Branch,1,1500,1500,,transfer";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "pharmacy_sales_template.csv");
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      Papa.parse(selectedFile, {
        ...
        complete: (results) => {
          ...
          const validData = results.data.filter((row: any) => row.DrugName && row.Quantity);
          if (validData.length === 0) {
            setError('No valid rows found. Please ensure "DrugName" and "Quantity" columns exist and have values.');
            return;
          }
    ...
    try {
      Papa.parse(file, {
        ...
        complete: async (results) => {
          try {
            const data = results.data.filter((row: any) => row.DrugName && row.Quantity);
            ...
            for (const row of data as any[]) {
              ...
              const newSalesRef = doc(collection(db, 'dispense_records'));
              
              const drugName = (row.DrugName || '').trim();
              const matchedDrug = drugs.find(d => d.name.toLowerCase() === drugName.toLowerCase());
              const drugId = matchedDrug ? matchedDrug.id : 'imported_unknown_id';
              
              const quantity = parseInt(row.Quantity, 10) || 0;
              const unitPrice = parseFloat(row.UnitPrice) || 0;
              const totalAmount = parseFloat(row.TotalAmount) || (quantity * unitPrice);
              
              // Try to parse the date, otherwise use current date
              ...
              const paymentMethod = (row.PaymentMethod || 'cash').toLowerCase();
              const validPaymentMethods = ['cash', 'card', 'transfer', 'insurance', 'other'];
              const finalPaymentMethod = validPaymentMethods.includes(paymentMethod) ? paymentMethod : 'other';

              currentBatch.set(newSalesRef, {
                branchId: 'imported_branch',
                branchName: (row.BranchName || 'Imported Data').trim(),
                drugId,
                drugName,
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <h2 className="text-lg font-bold text-slate-800">Import Past Sales Records</h2>
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh] flex flex-col gap-6">
          <div className="bg-blue-50 border border-blue-100 rounded-md p-4 text-sm text-blue-800">
            ...
            <p className="mb-3">Start by downloading our CSV template. Fill it with your past sales data, keeping the exact column headers. Date format should ideally be YYYY-MM-DD or ISO 8601.</p>
            <button 
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="p-2 font-medium text-slate-600">DrugName</th>
                      <th className="p-2 font-medium text-slate-600">Branch</th>
                      <th className="p-2 font-medium text-slate-600">Qty</th>
                      <th className="p-2 font-medium text-slate-600">TotalAmount</th>
                      <th className="p-2 font-medium text-slate-600">Payment</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {previewData.map((row, i) => (
                      <tr key={i}>
                        ...
                        <td className="p-2">{row.DrugName}</td>
