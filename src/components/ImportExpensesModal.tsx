...
export default function ImportExpensesModal({ isOpen, onClose, onSuccess, profile }: ImportExpensesModalProps) {
  ...
  const handleDownloadTemplate = () => {
    const csvContent = "data:text/csv;charset=utf-8,Date,Category,Amount,Description,PaymentMethod,ReferenceNumber\n2023-10-01,rent,500000,Facility Rent for October,transfer,REF12345\n2023-10-02,salaries,150000,Staff Wages,transfer,";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "pharmacy_expenses_template.csv");
      ...
      Papa.parse(selectedFile, {
        ...
        complete: (results) => {
          ...
          if (validData.length === 0) {
            setError('No valid rows found. Please ensure "Date" and "Amount" columns exist and have values.');
            return;
          }
    ...
    try {
      Papa.parse(file, {
        ...
        complete: async (results) => {
          try {
            const data = results.data.filter((row: any) => row.Date && row.Amount);
            ...
            for (const row of data as any[]) {
              ...
              const newExpenseRef = doc(collection(db, 'operating_expenses'));
              
              const amount = parseFloat(row.Amount) || 0;
              
              const rawCategory = (row.Category || 'other').toLowerCase();
              const validCategories = ['salaries', 'electricity', 'generator_fuel', 'generator_maintenance', 'rent', 'supplies', 'logistics', 'marketing', 'other'];
              const category = validCategories.includes(rawCategory) ? rawCategory : 'other';

              const paymentMethod = (row.PaymentMethod || 'transfer').toLowerCase();
              const validPaymentMethods = ['cash', 'transfer', 'card', 'cheque', 'other'];
              const finalPaymentMethod = validPaymentMethods.includes(paymentMethod) ? paymentMethod : 'other';

              // Try to parse Date as YYYY-MM-DD
              let expenseDate = row.Date;
              let createdAt = Date.now();
              const parsedDate = new Date(row.Date).getTime();
              if (!isNaN(parsedDate)) {
                createdAt = parsedDate;
                expenseDate = new Date(parsedDate).toISOString().split('T')[0];
              }

              currentBatch.set(newExpenseRef, {
                adminId: profile.id,
                adminName: profile.name,
                category,
                customCategory: category === 'other' ? rawCategory : '',
                amount,
                expenseDate,
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <h2 className="text-lg font-bold text-slate-800">Import Past Expenses</h2>
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh] flex flex-col gap-6">
          <div className="bg-blue-50 border border-blue-100 rounded-md p-4 text-sm text-blue-800">
            ...
            <p className="mb-3">Start by downloading our CSV template. Fill it with your past expense records. Valid categories include: salaries, electricity, generator_fuel, generator_maintenance, rent, supplies, logistics, marketing, other.</p>
            <button 
                ...
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      ...
                      <th className="p-2 font-medium text-slate-600">Amount</th>
                      <th className="p-2 font-medium text-slate-600">Description</th>
