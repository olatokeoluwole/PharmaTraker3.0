...
export default function HMOView({ profile, readOnly = false }: { profile: UserProfile, readOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    }, (err) => console.warn("HMOView drugs sync notice:", err));
    ...
    }, (err) => console.warn("HMOView prescriptions sync notice:", err));
  ...
  const handlePrescribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPatientName('');
      setQuantity(0);
      alert('Order authorized and sent to branch.');
    } catch (err) {
      console.error(err);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <section className="flex flex-col bg-white rounded-lg border border-slate-300 shadow-sm overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
            <h2 className="font-bold text-slate-700 uppercase text-xs">2. Order Form</h2>
          </div>
          <div className="p-4 flex-1">
            <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-100 space-y-3">
              <h3 className="text-xs font-bold text-indigo-800 mb-2">New Order</h3>
        ...
        <section className="flex flex-col bg-white rounded-lg border border-slate-300 shadow-sm overflow-hidden">
          <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
            <h2 className="font-bold text-slate-700 uppercase text-xs">Recent Orders</h2>
          ...
          <div className="p-4 flex flex-col flex-1 gap-2 max-h-[500px] overflow-y-auto">
            ...
            {myPrescriptions.length === 0 && (
              <div className="text-center text-xs text-slate-500 py-6">No orders issued yet.</div>
            )}
          </div>
        </section>
      </div>
